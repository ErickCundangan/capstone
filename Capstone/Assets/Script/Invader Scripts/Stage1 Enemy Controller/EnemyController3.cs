...
public class EnemyController3 : MonoBehaviour {
	...
	void FixedUpdate() {
		...
		if (!m1 && gameObject.transform.position.y >= 5) {
			gameObject.transform.position += new Vector3 (-1f, -1f, 0) * speed;
			if (gameObject.transform.position.y <= 5 || gameObject.transform.position.x <= -5)
				m1 = true;
		}
